...
\noindent
Some collections can also be modified through their iterator.
...
\begin{verbatim}
  Grow hierarchy           Shrink hierarchy              Replace hierarchy             Iterator hierarchy
    ...
    @UnknownGrow             @UnknownShrink               @UnknownReplace          @UnknownIteratorPolyShrink
      /      \                 /       \                    /       \                          |
 @Growable @Ungrowable   @Shrinkable @Unshrinkable   @Replaceable @Unreplaceable               |
      \      /                 \       /                    \       /                          |
    @BottomGrow             @BottomShrink                @BottomReplace              @IteratorPolyShrink
\end{verbatim}
  \end{minipage}
\end{smaller}
...
An assignment \code{T x = y} is valid only if the compile-type of \code{y} is a
subtype of (or equal to) \code{T} in \emph{every} hierarchy simultaneously.  For
...
Alias for \code{@Growable @Shrinkable @Replaceable} (but see XXXXX for exceptions).
...
Alias for \code{@Ungrowable @Unshrinkable @Unreplaceable} (but see XXXXX for exceptions).
The collection is definitely unmodifiable: calling any mutating operation
always throws \UnsupportedOperationException.
The checker issues a warning at every invocation of a grow,
...
\begin{description}
\item[\refqualclass{checker/modifiability/qual}{UnknownIteratorPolyShrink}]
The top qualifier.
...
\item[\refqualclass{checker/modifiability/qual}{IteratorPolyShrink}]
...
For example, \sunjavadoc{java.base/java/util/Queue.html}{\<Queue>} has
...
In general, a type is given the top qualifier when it has no method that
performs the given mutation.  For example, \<Queue> defaults to
\<@UnknownReplace>.  (In the above assignment, \<q> was flow-sensitively
refined to have type \<@Replaceable>.
...
java.base/java/util/Collections.html\#unmodifiableMap(java.util.Map)}{\code{Collections.unmodifiableMap()}}


Queue has no replace methods in general
...
% LocalWords:  UnknownIteratorPolyShrink IteratorPolyShrink
