...
2-element hierarchy to express whether a collection's \code{iterator}
 preserves the ability to call \code{Iterator.remove()}.
...
\begin{verbatim}
  Grow hierarchy           Shrink hierarchy              Replace hierarchy             Iterator hierarchy
    ...
    @UnknownGrow             @UnknownShrink               @UnknownReplace          @UnknownIteratorPreservesRemove
      /      \                 /       \                    /       \                          |
 @Growable @Ungrowable   @Shrinkable @Unshrinkable   @Replaceable @Unreplaceable               |
      \      /                 \       /                    \       /                          |
    @BottomGrow             @BottomShrink                @BottomReplace              @IteratorPreservesRemove
\end{verbatim}
  \end{minipage}
...
Calling grow operations such as \code{add}, \code{addAll}, etc.\ on this
collection will throw \code{UnsupportedOperationException}.
No guarantees are made about shrink or replace operations.
...
Calling shrink operations such as \code{remove}, \code{clear}, etc.\ on
this collection will throw \code{UnsupportedOperationException}.
No guarantees are made about grow or replace operations.
...
Calling replace operations such as \code{set}, \code{replaceAll}, etc.\
on this collection will throw \code{UnsupportedOperationException}.
No guarantees are made about grow or shrink operations.
...
\begin{description}
\item[\refqualclass{checker/modifiability/qual}{UnknownIteratorPreservesRemove}]
The top qualifier in the iterator-remove-preservation hierarchy.
The checker cannot determine whether calling \code{iterator()} on this
collection preserves the ability to call \code{Iterator.remove()}.
This is the default qualifier for unannotated types in the
iterator-remove-preservation hierarchy.

\item[\refqualclass{checker/modifiability/qual}{IteratorPreservesRemove}]
Calling \code{iterator()} on this collection preserves the receiver's
ability to remove elements: if the collection is \<@Shrinkable>, then
the resulting \code{Iterator} is \<@Shrinkable> and its
\code{remove} method may be called.
...
Alias for \code{@Ungrowable @Unshrinkable @Unreplaceable}.
...
\begin{Verbatim}
  ...
  @UnknownModifiability List<String> unknown = ...;  // capability is unknown
\end{Verbatim}
...
Useful for methods such as \code{Map.keySet()} that preserve Shrink while
...
\begin{Verbatim}
  // Shrink is preserved from the map; result is explicitly @Ungrowable.
  @PolyShrink @Ungrowable Set<K> keySet(@PolyShrink Map<K, V> map)
\end{Verbatim}
...
\sectionAndLabel{Types without grow, shrink, and/or replace methods}{types-without-full-gsr}
...
\begin{itemize}
    \item \code{@Modifiable Set} $\Rightarrow$
      treated as \code{@Growable @Shrinkable @UnknownReplace Set}
    \item \code{@Unmodifiable Set} $\Rightarrow$
      treated as \code{@Ungrowable @Unshrinkable @UnknownReplace Set}
...
\begin{itemize}
    \item \code{Map.@Modifiable Entry} $\Rightarrow$
      treated as \code{Map.@UnknownGrow @UnknownShrink @Replaceable Entry}
    \item \code{Map.@Unmodifiable Entry} $\Rightarrow$
      treated as \code{Map.@UnknownGrow @UnknownShrink @Unreplaceable Entry}
...
\begin{itemize}
    \item \code{@Modifiable Iterator} $\Rightarrow$
      treated as \code{@UnknownGrow @Shrinkable @UnknownReplace Iterator}
    \item \code{@Unmodifiable Iterator} $\Rightarrow$
      treated as \code{@UnknownGrow @Unshrinkable @UnknownReplace Iterator}
    \item \code{@Growable Iterator} $\Rightarrow$
      treated as \code{@UnknownGrow @UnknownShrink @UnknownReplace Iterator}
\end{itemize}

\sectionAndLabel{Iterator Behavior}{modifiability-iterator-behavior}

The iterator returned by a shrinkable collection does not always support
\code{Iterator.remove()}.  Therefore, when \code{iterator()} is called on
an \<@Shrinkable> receiver, the result is \<@Shrinkable> only if the
receiver is also \<@IteratorPreservesRemove>.  Otherwise, the result's
Shrink qualifier is conservatively \<@UnknownShrink>.  When
\code{iterator()} is called on an \<@Unshrinkable> receiver, the result
is \<@Unshrinkable>.

\begin{Verbatim}
  @IteratorPreservesRemove @Shrinkable List<String> list = ...;
...
% LocalWords:  UnknownIteratorPreservesRemove IteratorPreservesRemove
