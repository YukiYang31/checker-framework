...
In Java, an \emp{optional} method is one that a class may or may not
...
\item[\refqualclass{checker/modifiability/qual}{PolyShrink}]
    ...
    % MDE: Specify what happens if *both* a polymorphic and a
    % non-polymorphic qualifier are specified, such as both @PolyShrink and
    % @Modifiable.  Add tests to verify the behavior.
\end{description}
...
% MDE: I don't think that the notion of "capability" is necessary or
% useful.  It is just extra jargon that users need to understand.  Likewise
% for naming the bits as G, S, and R.

% MDE: Much of this section duplicates information that is present
% elsewhere.  Eliminate the duplication and put related information
% together.
